...
import {
  STAFF_STUDENT_COMMUNICATION_ROLES,
  generateStaffStudentCommunicationDraft,
  getStaffStudentCommunicationContext,
...
async function requireStaffUser() {
  ...
  if (!STAFF_STUDENT_COMMUNICATION_ROLES.includes(session.user.role)) {
    return { error: NextResponse.json({ error: "Forbidden" }, { status: 403 }) };
  }

  const displayName = session.user.name?.trim() || "Staff member";
...
export async function GET(request: Request) {
  ...
  try {
    const context = await getStaffStudentCommunicationContext(studentId, guard.staff);
    if (!context) {
    ...
    return NextResponse.json({
      ...context,
      stats: {
        contentLanguages: PARENT_COMMUNICATION_CONTENT_LANGUAGES,
        draftTypes: PARENT_COMMUNICATION_MESSAGE_TYPES.length,
      },
    });
  } catch (error) {
    console.error("[staff-student-communication] GET failed:", error);
    return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  try {
    const result = await generateStaffStudentCommunicationDraft(guard.staff, {
      childId,
      messageType,
      language,
      tone,
    ...
    console.error("[staff-student-communication] POST failed:", error);
    return NextResponse.json(
